...
옥천군 청산면 백운리 농촌형 교통 서비스 "다람쥐 택시"의 **프런트엔드**.
별도 백엔드 없이 **mock 데이터로 바로 도는 클릭형 프로토타입**입니다.
이장님·행정 담당자용 관리자 화면과 주민·가족용 예약 화면을 분리하되,
동일한 예약 데이터를 공유하도록 설계했습니다. 고령자 사용성과 전화 접수 시나리오가 최우선.
...
- mock(인메모리) 데이터라 DB 설정 없이 모든 화면이 클릭됩니다. (서버 재시작 시 초기화)
...
`lib/dal/index.ts`의 `db` 한 줄만 교체하면 됩니다. UI 코드 수정 불필요.
...
> 📝 검증된 빌드(`next build` 통과)이나 데이터는 mock 입니다.
